...
class MT2MBA_BACKEND_SETTINGS
{
    ...
    var $show_attrb_list    =   'no';       // The default behavior for showing attributes on the All Products page.
    ...
    /**
     * Set the Show Attributes on List option
     * @param   boolean $data   Whether the attributes will be shown in the All Products list
     * @return  boolean         Whether the attributes will be shown in the All Products list
     */
    private function set_show_attrb_list($data)
    {
        if($data === '')
        {
            $data = $this->round_markup;
        }
        if(update_option('mt2mba_show_attrb_list', $data))
        {
            return $data;
        }
        return FALSE;
     ...
     * @return  boolean             Whether percentage markups will be rounded
     */
    public function get_show_attrb_list()
    {
        $data = get_option('mt2mba_show_attrb_list');
        if(!isset($data))
        {
            $data = $this->set_show_attrb_list($this->show_attrb_list);
        }
        return $data;
    }
    ...
    function mt2mba_all_settings($settings, $current_section)
    {
        ...
        if($current_section == 'mt2mba')
        {
            ...
            // Attributes on All Products page
            register_setting('mt2mba', 'mt2mba_show_attrb_list');
            $description = __(
                "Should Markup-by-Attribute include a column on the 'All Products' page to show all attributes associated with the products? When set on, you can filter by attribute, making it easier to find products that contain attributes you may have changed.",
                'markup-by-attribute');
            $mt2mba_settings[] = array
                (
                    'title'    => __('Show Attributes on Product List', 'markup-by-attribute'),
                    'name'     => 'mt2mba_show_attrb_list',
                    'desc'     => sprintf($this->format_desc, $description),
